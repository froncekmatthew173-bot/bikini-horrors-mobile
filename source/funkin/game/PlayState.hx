...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	@:dox(hide) override public function create()
	{
		...
		Logs.infos('PlayState.create() finished in ${Std.int((Timer.stamp() - __createStartedAt) * 1000)}ms', "PlayState");
	}
	...
	override public function addExtraHitboxKey(key:String = "SPACE", ?buttonID:String):Dynamic
	{
		var button = super.addExtraHitboxKey(key, buttonID);
		var pauseButton = getMobilePadButton("buttonP");
		if (button != null && pauseButton != null && button.deadZones.indexOf(pauseButton) == -1)
	...
	@:dox(hide) public function startCountdown():Void
	{
		if (!_startCountdownCalled) {
			...
			inCutscene = false;
			if (!chartingMode && PlayStateLoadingState.lastStartedAt > 0) {
				var totalStartMs = Std.int((Timer.stamp() - PlayStateLoadingState.lastStartedAt) * 1000);
				Logs.infos('Countdown ready in ${totalStartMs}ms (loading ${Std.int(PlayStateLoadingState.lastLoadDurationMs)}ms)', "PlayState");
			}
	...
	**/
	...
	var __runtimeEffectsCleaned:Bool = false;

	function executeDueEvents() {
		if (events == null || events.length <= 0) return;

		var processed = 0;
		var lastBatchTime:Null<Float> = null;
		var budget = Flags.PLAYSTATE_EVENT_EXECUTION_BUDGET;
		if (budget <= 0) budget = 1;

		while(events.length > 0 && events.last().time <= Conductor.songPosition) {
			var nextEvent = events.last();
			if (processed >= budget && lastBatchTime != null && Math.abs(nextEvent.time - lastBatchTime) > 0.001)
				break;

			var event = events.pop();
			if (lastBatchTime == null || Math.abs(event.time - lastBatchTime) > 0.001)
				lastBatchTime = event.time;
	...
	public function cleanupRuntimeEffects(clearCameraFilters:Bool = true) @:privateAccess {
		if (__runtimeEffectsCleaned) return;
		__runtimeEffectsCleaned = true;
		...
		for (tween in eventsTween)
			if (tween != null)
				tween.cancel();
		eventsTween.clear();

		FlxG.timeScale = 1;
		if (inst != null) inst.pitch = 1;
		if (vocals != null) vocals.pitch = 1;
		if (strumLines != null)
			for (strumLine in strumLines.members)
				if (strumLine != null && strumLine.vocals != null)
					strumLine.vocals.pitch = 1;

		if (camFollow != null)
			FlxTween.cancelTweensOf(camFollow);

		clearCameraRuntime(camGame, clearCameraFilters);
		clearCameraRuntime(camHUD, clearCameraFilters);
	...
	public static function switchToPlayState(?skipTransition:Bool = true):Void {
		if (instance != null)
			instance.cleanupRuntimeEffects();
		...
		if (chartingMode)
		{
			PlayStateLoadingState.lastStartedAt = 0;
